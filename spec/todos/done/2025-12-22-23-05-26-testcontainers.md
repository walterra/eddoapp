...
Replace manual CouchDB setup with Testcontainers for integration/e2e tests. Uses best practice pattern: one container per test suite (starts in globalSetup, stops in globalTeardown), with fast database cleanup between individual tests. Developers will no longer need to manually start CouchDB. Additionally, split CI workflow into separate jobs for quality checks, unit tests, integration tests, and e2e tests for better parallelization and faster feedback. Success criteria: all tests pass without requiring external CouchDB instance, works locally and in CI.
...
- [x] Update GitHub Actions workflow (.github/workflows/test.yml)
  ...
  - Split into separate jobs: quality, unit-tests, integration-tests, e2e-tests
  - Run quality checks first, then run test jobs in parallel
...
- Split into 4 parallel jobs:
  - **quality**: Linting, formatting, TypeScript checks (fast feedback)
  - **unit-tests**: Fast unit tests, no external dependencies
  - **integration-tests**: Integration tests with testcontainers
  - **e2e-tests**: End-to-end tests with testcontainers
- Quality checks run first; test jobs run in parallel after quality passes
- Benefits: faster feedback, better parallelization, clearer failure reporting
