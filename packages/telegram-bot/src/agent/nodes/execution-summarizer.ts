...
import type { WorkflowNode, ExecutionSummary, ExecutionStep, WorkflowState } from '../types/workflow-types.js';

/**
 * Execution summarizer node - generates comprehensive summaries and next actions
 */
export const generateExecutionSummary: WorkflowNode = async (state: WorkflowState): Promise<Partial<WorkflowState>> => {
  ...
  try {
    ...
    await state.telegramContext.reply(summaryMessage, { parse_mode: 'Markdown' });
 ...
 */
async function createExecutionSummary(state: WorkflowState): Promise<ExecutionSummary> {
...
async function generateNextActions(state: WorkflowState): Promise<string[]> {
  ...
  const skippedSteps = state.executionSteps.filter((s) => s.status === 'skipped');
  if (skippedSteps.length > 0) {
    nextActions.push(`Address dependencies for ${skippedSteps.length} skipped step(s)`);
  }
 ...
 */
async function generateSummaryMessage(state: WorkflowState, summary: ExecutionSummary): Promise<string> {
  const duration = formatDuration(summary.duration);
  const successRate = Math.round((summary.completedSteps / summary.totalSteps) * 100);
...
function createBasicSummary(state: WorkflowState): ExecutionSummary {
  ...
  return {
    ...
    duration: Date.now() - startTime,
    changes: [`Executed ${steps.filter((s) => s.status === 'completed').length} actions successfully`],
