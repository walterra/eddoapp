...
class EnhancedApprovalManager {
  ...
  private approvalCallbacks: Map<string, (result: { approved: boolean; feedback?: string }) => void> = new Map();

  /**
   * Register a pending approval request
   * Called by the enhanced workflow when it hits an interrupt
   */
  registerPendingApproval(userId: string, request: ApprovalRequest, callback: (result: { approved: boolean; feedback?: string }) => void): void {
  ...
  cleanup(timeoutMs: number = 300000): void {
    ...
    if (cleanedCount > 0) {
      logger.info('Enhanced approval cleanup completed', {
        cleanedCount,
        remainingApprovals: Array.from(this.pendingApprovals.values()).flat().length,
      });
    }
  }
  ...
  getStatus(): {
    ...
    const totalPendingApprovals = Array.from(this.pendingApprovals.values())
      .flat().length;
