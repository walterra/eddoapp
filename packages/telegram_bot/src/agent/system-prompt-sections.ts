/**
 * System prompt template sections for the AI agent
 */
...
/**
 * Returns the communication style instructions
 */
export function getCommunicationStyleSection(): string {
  return `COMMUNICATION STYLE: You are communicating via Telegram chat. Keep responses CONCISE and BRIEF:
...
/**
 * Returns the date parsing instructions
 */
export function getDateParsingSection(): string {
  return `IMPORTANT: When parsing dates, ALWAYS convert natural language to ISO format (YYYY-MM-DDTHH:mm:ss.sssZ):
...
/**
 * Returns the memory request handling instructions
 */
export function getMemoryHandlingSection(): string {
  return `IMPORTANT: For memory requests (when user asks to "remember" something):
...
/**
 * Returns the URL handling instructions
 */
export function getUrlHandlingSection(): string {
  return `SPECIAL URL HANDLING: If the user's message contains only a URL (or URL with minimal text), automatically:
...
/**
 * Returns the MCP tool usage rules
 */
export function getMcpToolRulesSection(): string {
  return `CRITICAL: Follow each tool's parameter schema EXACTLY as defined. Each tool description includes usage examples showing the correct parameter format. Study the examples carefully and replicate the exact structure.
...
/**
 * Returns the bulk operations instructions
 */
export function getBulkOperationsSection(): string {
  return `BULK OPERATIONS: When performing bulk operations (updating/deleting multiple items):
...
/**
 * Returns the work request handling instructions
 */
export function getWorkRequestSection(): string {
  return `IMPORTANT: For "start working" requests (phrases like "let's start with", "begin with", "work on", "tackle"):
...
/**
 * Returns the context awareness instructions
 */
export function getContextAwarenessSection(): string {
  return `CONTEXT AWARENESS: Pay attention to the conversation history. If you previously:
...
/**
 * Returns the next action selection instructions
 */
export function getNextActionSection(): string {
  return `NEXT ACTION SELECTION: When users ask "what should I do next", "what to pick up", "what to work on", or similar:
...
/**
 * Returns the response format instructions
 */
